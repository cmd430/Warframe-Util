    ...
    unranked.forEach((warframe, index) => {
      if(!warframe.Name.includes(' Umbra Prime')) {
        ...
        if (warframe.Name.includes(' Prime')) {
          ...
          let armor = (rank30[index].Armor / warframe.Armor === wf_index.rank.armor ? null : rank30[index].Armor / warframe.Armor)
          let health = (rank30[index].Health / warframe.Health === wf_index.rank.health ? null : rank30[index].Health / warframe.Health)
          let shield = (rank30[index].Shields / warframe.Shields === wf_index.rank.shield ? null : rank30[index].Shields / warframe.Shields)
          let energy = (rank30[index].Energy / warframe.Energy === wf_index.rank.energy ? null : rank30[index].Energy / warframe.Energy)
          let strength
          ...
          if (armor !== null && !isNaN(armor)) {
            overrides.rank.armor = armor
          }
          if (health !== null && !isNaN(health)) {
            overrides.rank.health = health
          }
          if (shield !== null && !isNaN(shield)) {
            overrides.rank.shield = shield
          }
          if (energy !== null && !isNaN(energy)) {
            overrides.rank.energy = energy
          }
          if (strength !== null && !isNaN(strength)) {
            overrides.rank.strength = strength
          }
          if (Object.keys(overrides.rank).length !== 0) {
